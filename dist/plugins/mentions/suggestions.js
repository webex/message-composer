...
const Suggestions = ({
  ...
  const onSelection = item => {
    ...
    const [, inputValue] = getInput(value); // Delete the captured value, including the `@` symbol

    editor.deleteBackward(inputValue.length + 1);
    const selectedRange = editor.value.selection;
    editor.insertText(' ').insertInlineAtRange(selectedRange, {
      data: { ...item,
  ...
  if (!state.disabled && state.items.length) {
    ...
    items = state.items.map((item, index) => {
      ...
      const {
        key,
        render
      } = mentions.renderSuggestion(item, {
        active
      });
      const itemProps = {
        onMouseDown: e => {
      ...
      return React.createElement("div", itemProps, render);
    });
  } else {
    setFlags({
      open: false
  ...
  const suggestionsStyles = { ...styles,
  ...
  return React.createElement(React.Fragment, null, children({
    ref: anchorRef
  }), React.createElement(Portal, null, React.createElement("div", {
